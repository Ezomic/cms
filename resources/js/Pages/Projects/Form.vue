...
interface ProjectData {
    id: number;
    name: string; slug: string | null; client_name: string | null; year: string | null;
    github_url: string | null; description: string | null; outcome: string | null; body: string | null;
    tags: string | null; published: boolean; meta_title: string | null; meta_description: string | null;
            ...
            <div class="flex flex-col gap-5">
                ...
                <Card>
                    ...
                    <CardContent class="flex flex-col gap-4">
                        ...
                        <template v-else>
                            <div><Label for="desc_nl">Korte omschrijving</Label><Textarea id="desc_nl" v-model="form.description_nl" /></div>
                            <div><Label for="body_nl">Body (case study)</Label><Textarea id="body_nl" v-model="form.body_nl" :rows="8" /></div>
                ...
                <Card>
                    ...
                    <CardContent class="flex flex-col gap-4">
                        <div>
                            ...
                            <label class="flex cursor-pointer flex-col items-center gap-2 rounded-md border border-dashed border-input bg-background px-4 py-6 text-center text-sm text-muted-foreground hover:border-primary">
                                <img v-if="imagePreview" :src="imagePreview" alt="" class="mb-1 h-28 rounded-md border border-border object-cover" />
                        ...
                        <div v-if="project?.images.length">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <div v-for="img in project.images" :key="img.id" class="relative">
                                    <img :src="img.url" alt="" class="h-16 w-24 rounded-md border border-border object-cover" :class="form.remove_images.includes(img.id) ? 'opacity-30' : ''" />
                                    <button type="button" class="absolute -right-1.5 -top-1.5 flex size-5 items-center justify-center rounded-full bg-destructive text-destructive-foreground" @click="toggleRemove(img.id)"><X class="size-3" /></button>
                                </div>
                            </div>
                        </div>
                        <div>
                            <Label for="gallery">Add gallery images</Label>
                            <input id="gallery" type="file" accept="image/*" multiple class="block w-full text-sm text-muted-foreground file:mr-3 file:rounded-md file:border file:border-input file:bg-card file:px-3 file:py-1.5 file:text-sm" @change="onGallery" />
                            <p v-if="form.errors.gallery" class="mt-1 text-xs text-destructive">{{ form.errors.gallery }}</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
                ...
                <Card>
                    <CardContent class="flex flex-col gap-4">
                        ...
                        <div>
                            ...
                            <p v-if="form.errors.github_url" class="mt-1 text-xs text-destructive">{{ form.errors.github_url }}</p>
                        </div>
                        <div>
...
.lt {
    @apply cursor-pointer rounded-md border border-border px-2.5 py-1 font-mono text-[11px] text-muted-foreground;
}
.lt-on {
