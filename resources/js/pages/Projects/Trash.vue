            ...
            <Table>
                ...
                <TableBody>
                    <TableRow v-for="row in projects" :key="row.id">
                        ...
                        <TableCell><span class="font-mono text-xs text-muted-foreground">{{ row.deleted_at }}</span></TableCell>
                        <TableCell>
                            <div class="flex justify-end gap-2">
                                <Button variant="outline" size="sm" @click="restore(row)">Restore</Button>
                                <Button variant="ghost" size="sm" class="text-destructive hover:bg-destructive/10 hover:text-destructive" @click="forceDelete(row)">Delete forever</Button>
                            </div>
                        </TableCell>
                    </TableRow>
                </TableBody>
            </Table>
            <p v-if="!projects.length" class="px-5 py-10 text-center text-sm text-muted-foreground">Trash is empty.</p>
        </Card>
    </AdminLayout>
</template>
