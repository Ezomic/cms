...
<template>
    <Card>
            ...
            <TableBody>
                <TableRow v-for="i in items" :key="i.id">
                    <TableCell>
                        ...
                        <div v-if="i.subtitle" class="text-muted-foreground font-mono text-[11px]">{{ i.subtitle }}</div>
                    </TableCell>
                    <TableCell
                        ><span class="text-muted-foreground font-mono text-xs">{{ i.deleted_at }}</span></TableCell
                    >
                    <TableCell>
        ...
        <p v-if="!items.length" class="text-muted-foreground px-5 py-10 text-center text-sm">Trash is empty.</p>
    </Card>
</template>
